...
float Balle::calculateDensity(std::vector<Balle> &balles,
...
sf::Vector2f Balle::calculatePressureForce(
    const std::vector<Balle> &balles, int particleIndex, float smoothingRadius,
    float mass, float targetDensity, float pressureMultiplier,
  ...
  foreachPointInRadius(
      pos, smoothingRadius, balles, spatialLookup, startIndices, numParticles,
      [&](int neighborIdx) {
        if (particleIndex != neighborIdx) {
          ...
          if (dstsqrt > 0) {
            ...
            float sharedPressure = calculateSharedPressure(
                density, balles[neighborIdx].getDensity(), targetDensity,
                pressureMultiplier);
      ...
      [&](int neighborIdx) {
        if (particleIndex != neighborIdx) {
          ...
          if (dstsqrt > 0) {
            ...
            viscosityForce +=
                (balles[neighborIdx].getVelocity() - velocity) * slope;
          }
        }
      },
      tmpAffected);
...
float Balle::convertDensityToPressure(float density, float targetDensity,
...
void Balle::updateDensity(
    const std::vector<Balle> &balles, float smoothingRadius, int index,
    float mass, const std::vector<std::pair<unsigned int, int>> &spatialLookup,
    const std::vector<unsigned int> &startIndices, int numParticles) {
  density = 0.0f;
  sf::Vector2f pos = getPredPosition();
  std::vector<int> tmpAffected;
...
float Balle::calculateSharedPressure(float densityA, float densityB,
                                     float targetDensity,
                                     float pressureMultiplier) {
  float pressureA =
      convertDensityToPressure(densityA, targetDensity, pressureMultiplier);
  float pressureB =
      convertDensityToPressure(densityB, targetDensity, pressureMultiplier);
...
void updateSpatialLookup(
  ...
  for (int i = 0; i < numParticles; i++) {
    std::pair<int, int> cell =
        positionToCellCoord(balles[i].getPosition(), radius);
