...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (!paused) {
            ...
            for (int index = 0; index < numParticles; index++) {
                pressureForces[index] = balles[index].calculatePressureForce(
                    balles, index, smoothingRadius, mass, targetDensity, pressureMultiplier,
                    spatialLookup, startIndices, numParticles);
                viscosityForces[index] = balles[index].calculateViscosityForce(
                    balles, index, smoothingRadius, viscosity,
                    spatialLookup, startIndices, numParticles);
        ...
        if (showCircle && selectedParticle >= 0 && selectedParticle < balles.size()) {
            ...
            ImGui::Text("Force de Pression: (%.5f, %.5f)", pressureForce.x, pressureForce.y);
        ...
        // Récupération des indices des balles interagissant avec la balle sélectionnée
        std::vector<int> affectedBalles;
        if (showCircle && selectedParticle >= 0 && selectedParticle < balles.size()) {
            affectedBalles.clear();
            foreachPointInRadius(
                balles[selectedParticle].getPredPosition(),
                smoothingRadius,
                balles,
                spatialLookup,
                startIndices,
        ...
        for (int i = 0; i < balles.size(); i++) {
            if (showCircle) {
                ...
                } else if (std::find(affectedBalles.begin(), affectedBalles.end(), i) != affectedBalles.end()) {
